 ...
 */
export default function SettingsDashboard({partyID, id, name, teams, dances, judges }: { partyID: string; id: string; name: string; teams: Team[]; dances: Dance[]; judges: Judge[] }) {
   ...
   */
  const handleChange = async <K extends keyof EventData>(val: EventData[K], field: K) => {
    try {
      if (!id) return;
      await updateDoc(doc(collection(doc(collection(db, 'parties'), partyID), 'events'), id), {
        [field]: val,
      });
    } catch (err) {
  ...
  const addTeam = () => {
    const newTeam: Team = { id: crypto.randomUUID(), name: '', color: '#8b5cf6', logo: '' };
  ...
  return (
    <div className="space-y-8 pb-12 max-w-5xl mx-auto">
      <div>
        ...
        <p className="mt-2 text-stone-500 text-lg">Configure your event details, teams, and judges.</p>
      </div>
      ...
      <div className="bg-white shadow-sm sm:rounded-3xl p-8 border border-stone-200/60">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={addTeam} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-full text-white bg-violet-600 hover:bg-violet-700 shadow-sm transition-colors">
            <Icon name="Plus" className="mr-2 h-4 w-4" /> Add Team
          </button>
        </div>
        <div className="space-y-4">
          {teams.length === 0 && <p className="text-stone-500 italic">No teams added yet.</p>}
          {teams.map((team, index) => (
            <div key={team.id} className="flex items-center space-x-4 p-4 border border-stone-200 rounded-2xl bg-stone-50/50 hover:bg-white transition-colors">
              <input
                type="text"
                value={team.name}
              ...
              <Image
                                ...
                                />
              <button onClick={() => deleteTeam(index)} className="p-2 text-stone-400 hover:text-red-600 hover:bg-red-50 rounded-full transition-colors">
      ...
      <div className="bg-white shadow-sm sm:rounded-3xl p-8 border border-stone-200/60">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={addDance} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-full text-white bg-violet-600 hover:bg-violet-700 shadow-sm transition-colors">
            <Icon name="Plus" className="mr-2 h-4 w-4" /> Add Dance
          </button>
        </div>
        <div className="space-y-4">
          {dances.length === 0 && <p className="text-stone-500 italic">No dances added yet.</p>}
          {dances.map((dance, index) => (
            <div key={dance.id} className="flex items-center space-x-4 p-4 border border-stone-200 rounded-2xl bg-stone-50/50 hover:bg-white transition-colors">
              <input
                type="text"
                value={dance.name}
                ...
                className="flex-1 rounded-xl border-stone-300 shadow-sm focus:border-violet-500 focus:ring-violet-500 sm:text-sm p-2.5 border"
              />
              <button onClick={() => deleteDance(index)} className="p-2 text-stone-400 hover:text-red-600 hover:bg-red-50 rounded-full transition-colors">
                <Icon name="Trash2" className="h-5 w-5" />
              </button>
            </div>
          ))}
      ...
      <div className="bg-white shadow-sm sm:rounded-3xl p-8 border border-stone-200/60">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={addJudge} className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-full text-white bg-violet-600 hover:bg-violet-700 shadow-sm transition-colors">
            <Icon name="Plus" className="mr-2 h-4 w-4" /> Add Judge
          </button>
        </div>
        <div className="space-y-4">
          {judges.length === 0 && <p className="text-stone-500 italic">No judges added yet.</p>}
          {judges.map((judge, index) => (
            <div key={judge.id} className="flex items-center space-x-4 p-4 border border-stone-200 rounded-2xl bg-stone-50/50 hover:bg-white transition-colors">
              <div className="h-12 w-12 rounded-full overflow-hidden bg-stone-200 flex-shrink-0">
                {judge.image ? (
                  <Image src={judge.image} alt={judge.name} width={48} height={48} className="h-full w-full object-cover" />
                ) : (
              ...
              <input
                ...
                className="flex-1 rounded-xl border-stone-300 shadow-sm focus:border-violet-500 focus:ring-violet-500 sm:text-sm p-2.5 border"
              />
              <button onClick={() => deleteJudge(index)} className="p-2 text-stone-400 hover:text-red-600 hover:bg-red-50 rounded-full transition-colors">
                <Icon name="Trash2" className="h-5 w-5" />
              </button>
            </div>
          ))}
      ...
      {isJudgeModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full p-8 transform transition-all max-h-[80vh] flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-bold text-stone-900">Select a Judge</h3>
              <button onClick={() => setIsJudgeModalOpen(false)} className="text-stone-400 hover:text-stone-600 bg-stone-100 hover:bg-stone-200 p-2 rounded-full transition-colors">
                  ...
                  {availableJudges.map((judge) => (
                    ...
                    >
                      ...
                      <div className="truncate">
                        <p className="font-bold text-stone-900 truncate">{judge.name}</p>
                        <p className="text-xs text-stone-500 truncate">ID: {judge.id}</p>
                      </div>
                    </button>
                  ))}
                </div>
